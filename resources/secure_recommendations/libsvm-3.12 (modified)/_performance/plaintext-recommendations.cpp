...
*/
unsigned long CpuTimer::GetDuration () const
{
	return static_cast<unsigned long>((clock() - this->timer) * 1000 / CLOCKS_PER_SEC);
}

/**
@param input the input values
@return the mean @f$ \bar{x} = \frac{1}{n} \cdot \sum_{i=1}^n{x_i} @f$
*/
double getMean (const std::vector<long> &input)
{
	double mean = 0.0;

	for (std::vector<long>::const_iterator i = input.begin(); i != input.end(); ++i)
	{
...
/**
@param input the input values
@param mean the mean of the input values
@return The standard deviation: @f$ \sigma=\sqrt{\frac{\sum_{i=1}^n a_i^2}n - \left(\frac{\sum_{i=1}^n a_i}n\right)^2} @f$
*/
double getStandardDeviation (const std::vector<long> &input, const double &mean)
{
	double standardDeviation = 0.0;

	for (std::vector<long>::const_iterator i = input.begin(); i != input.end(); ++i)
	{
		standardDeviation += std::pow(static_cast<double>(*i) - mean, 2);
	}

	return std::sqrt(standardDeviation / static_cast<double>(input.size()));
}
/**
@param input the input values
@return The standard deviation: @f$ \sigma=\sqrt{\frac{\sum_{i=1}^n a_i^2}n - \left(\frac{\sum_{i=1}^n a_i}n\right)^2} @f$
*/
double getStandardDeviation (const std::vector<long> &input)
{
	double mean = getMean(input);

	return getStandardDeviation(input, mean);
}
/* /helpers */
...
int main ()
{
	...
	std::string testDataDirectory = "/tudelft.net/staff-bulk/ewi/mm/ISPLab/mtodor/results/input/";
	std::string medicalRelevanceTestDataFile = "philips.test.medical";
	std::string medicalRelevanceModelsDirectory = "/tudelft.net/staff-bulk/ewi/mm/ISPLab/mtodor/results/models/medical_relevance/" + kernel + "/";
	std::string safetyModelsDirectoy = "/tudelft.net/staff-bulk/ewi/mm/ISPLab/mtodor/results/models/safety/" + kernel + "/";
	...
	try
	{
		...
		for (size_t row = 0; row < medicalRelevanceTestData.x.size(); ++row)
		{
			...
			//evaluate the safety block
			std::vector<bool> safetyPredictionsVector;
			for (std::vector<std::shared_ptr<Svm>>::const_iterator safetySVMIterator = safetySvmVector.begin(); safetySVMIterator != safetySvmVector.end(); ++safetySVMIterator)
			{
				//we're only interested in the sign of the prediction
				safetyPredictionsVector.push_back(getLabel(safetyBlockTestDataMap[safetySVMIterator->get()->GetUnsafeClasses()]->x[row], *safetySVMIterator));
			}
		...
		std::cout << "Entire process duration: " << mainTimer.GetDuration() << std::endl;
		
		/*
		//compute minimum and maximum durations
		long minimum = durationVector[0];
		long maximum = durationVector[0];
		
		//start from 1
		for (size_t i = 1; i < durationVector.size(); ++i)
		{
		...
		std::cout << "Minimum duration (clock tick counts): " << minimum << std::endl;
		std::cout << "Maximum duration (clock tick counts): " << maximum << std::endl;

		double mean = getMean(durationVector);
		std::cout << "Average duration (clock tick counts): " << mean << std::endl;
		std::cout << "Standard deviation (clock tick counts): " << getStandardDeviation(durationVector, mean) << std::endl;
		*/
		std::cout << std::endl;
