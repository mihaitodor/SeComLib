...
#define SKIP_ELEMENT\
	...
	while(readline(fp)!=NULL)
	{
		...
		/* +Michael - preserve the line labels, if they exist, but just for the test data files; the model files only contain the "gen" label, which we ignore */
		if (strchr(p, '#') && strncmp(p, " # gen:", 7))
		{
			printf("%s", p + 1);//skip the first space
		}
		/* the above printf already outputs the line terminator so we don't want to print it twice*/
		else
		/* /+Michael */
